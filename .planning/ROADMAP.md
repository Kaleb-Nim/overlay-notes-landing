...
- [x] **Phase 3: SEO Metadata, Structured Data & Social Card** - Correct absolute-URL `<head>`, `SoftwareApplication` JSON-LD, robots/sitemap, and a static OG image verified in LinkedIn's Post Inspector (completed 2026-07-25)
...
**Success Criteria** (what must be TRUE):
  ...
  3. The primary CTA links to the live Chrome Web Store listing (`https://chromewebstore.google.com/detail/overlay-notes/ogekdbffoapphpabjphfgeppildcleck`); the secondary CTA links to the GitHub repo; the support CTA links to `https://buymeacoffee.com/kalebnim`, and the $3/$8/$20 tip chips are either real links or visually non-interactive — never dead elements styled to look clickable.
...
- [x] 03-02-PLAN.md — OG social card: committed fonts + `scripts/generate-og-image.ts` (Playwright bundled-Chromium screenshot) → `public/og-image.png` at 1200×630, `generate:og` script (ASSET-01/02 structural)

**Wave 2** *(blocked on Wave 1 completion)*

- [x] 03-03-PLAN.md — SEO test suite goes live: extend `tests/seo.spec.ts` to assert (no silent skips) + 4 new checks, full green gate, landing.spec.ts unweakened
...
- [x] 04-01-PLAN.md — Pre-publish audit + README + deployment verifier; publish the repo publicly, wire the Vercel project via `vercel git connect`, verify the first production deploy on its `*.vercel.app` URL, and prove PR preview deployments (DEPL-01, DEPL-02)

**Wave 2** *(blocked on Wave 1 — the ROADMAP's verify-before-attach ordering is a real dependency)*

- [x] 04-02-PLAN.md — Attach `overlay-notes.kalebnim.dev`, read the project-specific CNAME live, human checkpoint at Google Cloud DNS, then verify HTTPS + valid cert + no leftover noindex header and an unharmed zone (DEPL-03; closes Phase 3's deferred SEO-08 item)

...
- `playwright.config.ts`, `scripts/test-gate.sh`, `tests/landing.spec.ts`, `tests/seo.spec.ts`, `TESTING.md` already exist and MUST survive scaffolding. **`create-next-app` REFUSES to run here** (confirmed by Phase 1 research — it names `TESTING.md`/`playwright.config.ts`/`scripts/`/`tests/` as blocking conflicts, no bypass flag). So scaffold **manually**: `bun add next react react-dom` + `bun add -d typescript @types/react @types/node`, then hand-author the 5 config/app files (`next.config`, `tsconfig.json`, `next-env.d.ts`, `app/layout.tsx`, `app/page.tsx`, `app/globals.css`). Verified end-to-end with a real `bun run build` on Next 16.2.11 + Turbopack.
- **DECISION (2026-07-24, user-confirmed): DEFER the Playwright install to Phase 2 — do NOT install it in Phase 1.** Rationale: `scripts/test-gate.sh` goes live the instant `@playwright/test` is installed (its other two conditions — `playwright.config.ts` and `tests/*.spec.ts` — are already satisfied in the repo), and `tests/landing.spec.ts` hard-asserts Phase-2 surfaces (CTAs, images) with no self-guard, so a Phase-1 install would make Phase 1 fail its own execute-phase regression gate. Leaving Playwright uninstalled keeps the gate inert (exit 0) through Phase 1 — matching `test-gate.sh`'s own header comment and `TESTING.md` ("active from Phase 2 on"; setup "Phase 1 / early Phase 2"). This intentionally supersedes CLAUDE.md's literal "install during Phase 1 setup" wording. Phase 2 installs Playwright as its first step. Still add `"test": "playwright test"` to `package.json` scripts in Phase 1 (harmless; the GSD gate calls `scripts/test-gate.sh`, not `bun test`).

### Font-weight correction (from Phase 1 research — supersedes the weight list above)
- Load the **superset** for Shantell Sans: `weight: ['400','500','600','700'], style: ['normal','italic']`. Reason: `concept-1a.html` uses Shantell **500 _normal_** in 5 real elements (`.note-r`, `.note-b`, `.who .kicker`, `.origin p`, the support-section note), but the earlier weight list only captured 500 as _italic_ (which renders nowhere). `next/font/google` fetches `weight × style` as a full cartesian product; unused `@font-face` declarations are not downloaded, so requesting the superset costs nothing at runtime and resolves both readings. Public Sans similarly: `weight: ['400','500','600','700'], style: ['normal','italic']`.

### Banned-term scan — known false positives to whitelist (from research)
- The scan for banned terms MUST NOT flag: (a) "accounts" inside the REQUIRED support claim "no accounts, no ads, nothing tracked" (CONT-08) — it's a negation, the opposite of claiming an accounts feature; (b) "shared" inside the certified privacy statement "…sold, or shared" (CONT-03) — distinct from the banned feature-term "sharing". Implement the check with word-boundary/context awareness or an explicit allowlist of these two locked phrases, so the traceability check passes on correct copy.

### faqs[] shape clarification (from research)
- `faqs[]` is `{ question, answer }[]` consumed by Phase 2's **visible FAQ only**. The earlier "also feeds the JSON-LD builder" note is stale: `FAQPage` JSON-LD is explicitly banned (REQUIREMENTS.md Out of Scope — Google removed FAQ rich results 2026-05-07), and Phase 3's JSON-LD is `SoftwareApplication`, which does not consume `faqs[]`. Keep the single-source-of-truth shape for the visible FAQ; drop the JSON-LD-consumer framing.
